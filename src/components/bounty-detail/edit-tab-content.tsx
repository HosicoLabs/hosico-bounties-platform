...
export function EditTabContent({ bounty, onBountyUpdated }: EditTabContentProps) {
    ...
    const isBountyFinished = isEnded(bounty?.end_date)
    ...
    useEffect(() => {
        if (bounty) {
            ...
            if (bounty.end_date) {
                ...
                if (formattedDate < today && !isBountyFinished) {
    ...
    return (
        <Card className="border-0 shadow-lg bg-white/80 backdrop-blur-sm">
            <CardHeader>
                <CardTitle className="text-[#1c398e]">
                    {isBountyFinished ? "Bounty Details (Finished)" : "Edit Bounty"}
                </CardTitle>
                <CardDescription>
                    {isBountyFinished
                        ? "This bounty has finished and cannot be edited"
                        : "Update bounty details and requirements"}
                </CardDescription>
            </CardHeader>
                ...
                {isBountyFinished && (
                    <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                        <p className="text-sm text-yellow-800 font-medium">
                            This bounty has ended and is now read-only. You cannot make changes to finished bounties.
                        </p>
                    </div>
                )}
                        ...
                        <div className="space-y-4 mb-4">
                            <div>
                                ...
                                <Input
                                    ...
                                    required
                                    disabled={isSubmitting || isBountyFinished}
                                    readOnly={isBountyFinished}
                                    value={bountyTitle}
